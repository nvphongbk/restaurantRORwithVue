<template>
  <a-form-model
    ...
    <a-form-model-item label="Tên thành phần" prop="name">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      create(item) {
        this.isEdit = false
          return ApiCaller().post(URLS.MAIN_INGREDIENTS(), {
          ...
          if (valid) {
            ...
            if (this.isEdit) {
              let idItem = this.editItem.id
                ApiCaller().put(URLS.MAIN_INGREDIENT(idItem), {
                  main_ingredient: valuesSave
                })
                .then(response => {
                  this.$message.success('Cập nhật thành công')
                  this.$emit('updateListAfterUpdated', this.editItem);
